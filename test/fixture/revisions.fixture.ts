...
export class RevisionsFixture {
  static createRevisionEntity(
    submission: SubmissionsEntity,
    partial?: Partial<RevisionsEntity>,
    index: number = 0,
  ): RevisionsEntity {
    const entity = new RevisionsEntity();
    entity.submission = submission;
    entity.status = partial?.status ?? SubmissionStatus.PENDING;
    entity.componentType = partial?.componentType ?? `comp_${index}_${faker.string.alpha({ length: 10 })}`;
    ...
    entity.createdDt = partial?.createdDt ?? new Date();
